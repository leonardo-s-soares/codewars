...
on most nights, scraps on Saturdays, lentils on Fridays, and a pigeon or 
so extra on Sundays, made away with three-quarters of his income.")
# => ["a", "of", "on"]
...
Bonus points (not really, but just for fun):
    ...
    Avoid creating an array whose memory footprint is roughly as
    big as the input text.
...
def top_3_words(text=''):
    ...
    for index in range(0, len(text)):
        if is_valid_letter(index, text) or is_valid_apostrophe(index, text):
            result += text[index]
        else:
            result += ' ' 
    return list(dict(Counter(result.split()).most_common(3)).keys())

def is_valid_letter(index, text):
    if text[index].isalpha() or text[index].isspace():
        return True
    else:
        return False

def is_valid_apostrophe(index, text):
    if text[index] == "'":
        if index != 0 and index <= len(text)-1:
            if text[index-1].isalpha() and text[index] == "'":
                return True
            elif text[index+1].isalpha() and text[index] == "'":
                return True
    else:
        return False
